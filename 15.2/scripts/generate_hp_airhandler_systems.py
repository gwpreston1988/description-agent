...
# System combinations (HP + Air Handler)
systems = [
    ('GLZS5BA1810', 'AMST24BU13'),  # 1.5 ton
    ('GLZS5BA2410', 'AMST24BU13'),  # 2.0 ton
    ('GLZS5BA3010', 'AMST30BU13'),  # 2.5 ton
    ('GLZS5BA3610', 'AMST36BU13'),  # 3.0 ton
    ('GLZS5BA4210', 'AMST42CU13'),  # 3.5 ton
    ('GLZS5BA4810', 'AMST48CU13'),  # 4.0 ton
    ('GLZS5BA6010', 'AMST60DU13'),  # 5.0 ton
]
...
for hp_model, ah_model in systems:
    ...
    hp_eer2 = hp_data['eer2']
    hp_hspf2 = hp_data['hspf2']
    hp_cooling_capacity = hp_data['cooling_capacity_btuh']
    hp_heating_capacity = hp_data['heating_capacity_btuh']
    hp_sound = hp_data['sound_level_dba']
    hp_voltage = hp_data['voltage']
    hp_refrigerant = "R-410A"
    hp_defrost = hp_data['defrost_type']
    hp_low_ambient = hp_data['low_ambient_cooling']
    hp_warranty = hp_data.get('warranty_years', '10')
    ...
    ah_blower_speeds = ah_data.get('blower_speeds', 'Variable')
    ah_config = ah_data.get('configuration', 'Multi-Position')
...
<head>
    ...
    <title>GOODMAN {tonnage_display} Ton {hp_seer2} SEER2 R-410A Heat Pump System with Multi-Positional Air Handler</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>GOODMAN {tonnage_display}.0 Ton Up to {hp_seer2} SEER2 R-410A Heat Pump System with Multi-Positional Air Handler</h1>
        
        <div class="intro">
            <p>This complete Goodman heat pump system delivers reliable year-round comfort with efficient cooling and heating performance. Featuring a {hp_seer2} SEER2 heat pump paired with a multi-positional air handler, this matched system is engineered for optimal performance and longevity.</p>
        </div>
        ...
        <div class="tabs">
            <button class="tab-button" onclick="showTab(event, 'hp-specs')">Heat Pump</button>
            <button class="tab-button" onclick="showTab(event, 'ah-specs')">Air Handler</button>
            <button class="tab-button" onclick="showTab(event, 'lineset-specs')">Line Set</button>
        </div>
        
        <div id="hp-specs" class="tab-content">
            <table class="spec-table">
                <tr>
                    <th colspan="2">Heat Pump Specifications - {hp_model}</th>
                </tr>
                <tr>
                    <td>Cooling Capacity</td>
                    <td>{hp_cooling_capacity:,} BTU/h</td>
                </tr>
                <tr>
                    <td>Heating Capacity</td>
                    <td>{hp_heating_capacity:,} BTU/h at 47°F</td>
                </tr>
                <tr>
                    <td>SEER2 Rating</td>
                    <td><span class="highlight">{hp_seer2}</span></td>
                </tr>
                <tr>
                    <td>EER2 Rating</td>
                    <td>{hp_eer2}</td>
                ...
                <tr>
                    <td>HSPF2 Rating</td>
                    <td><span class="highlight">{hp_hspf2}</span></td>
                </tr>
                <tr>
                    <td>Refrigerant Type</td>
                    <td>{hp_refrigerant}</td>
                </tr>
                <tr>
                    <td>Defrost Type</td>
                    <td>{hp_defrost}</td>
                ...
                <tr>
                    <td>Warranty</td>
                    <td>{hp_warranty}-Year Limited Warranty</td>
                </tr>
            </table>
        </div>
        ...
        <div id="ah-specs" class="tab-content">
            <table class="spec-table">
                <tr>
                    <th colspan="2">Air Handler Specifications - {ah_model}</th>
                </tr>
                <tr>
                    <td>Airflow</td>
...
print(f"\nSuccessfully generated {len(systems)} heat pump system descriptions!")
