...
def parse_fraction(value: str) -> Optional[float]:
    ...
    # Handle mixed fractions like "16⁵⁄₁₆" or "16⁵⁄16"
    ...
    # Handle standard fractions like "5/16" or "⁵⁄₁₆"
    # First normalize superscript/subscript fractions
    value = value.replace('⁄', '/').replace('₁', '1').replace('₂', '2').replace('₃', '3')
    value = value.replace('₄', '4').replace('₅', '5').replace('₆', '6').replace('₇', '7')
    value = value.replace('₈', '8').replace('₉', '9').replace('₀', '0')
    ...
    if '/' in value:
        try:
            # Handle mixed numbers like "16 5/16" or "165/16"
            if ' ' in value:
                parts = value.split()
                whole = float(parts[0])
                frac = parts[1].split('/')
...
def parse_air_handler(pdf_path: Path, model_number: str) -> Dict:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        # Page 4 - Dimensions table
        if len(pdf.pages) > 3:
            dim_page = pdf.pages[3]
            dim_tables = dim_page.extract_tables()

            if dim_tables and len(dim_tables) > 0:
                dim_table = dim_tables[0]

                # Find the row with our model (with wildcard)
                # Handle both AMST60 and AMST06 variants
                model_base = model_number.replace('13', '').replace('14', '')
                model_search = model_base.replace('BU', 'BU*').replace('CU', 'CU*').replace('DU', 'DU*')
                # Also try with leading zero swapped (60 <-> 06)
                model_alt = None
                if '60' in model_base:
                    model_alt = model_base.replace('60', '06').replace('BU', 'BU*').replace('CU', 'CU*').replace('DU', 'DU*')
                elif '06' in model_base:
                    model_alt = model_base.replace('06', '60').replace('BU', 'BU*').replace('CU', 'CU*').replace('DU', 'DU*')

                for row in dim_table[1:]:  # Skip header
                    if row and row[0]:
                        if model_search in row[0] or model_base in str(row[0]) or (model_alt and model_alt in row[0]):
                            # Columns: Model, A" (height), B" (width), C" (depth), ...
                            if len(row) >= 4:
                                spec['height_in'] = parse_fraction(row[1])
                                spec['width_in'] = parse_fraction(row[2])
                                spec['depth_in'] = parse_fraction(row[3])
                            break
...
def parse_wall_hung_air_handler(pdf_path: Path, model_number: str) -> Dict:
    ...
    # Typical dimensions for wall-hung units (compact design)
    if spec['tonnage']:
        if spec['tonnage'] <= 1.5:
            spec['height_in'] = 28.0
            spec['width_in'] = 35.0
            spec['depth_in'] = 10.0
        elif spec['tonnage'] <= 2.5:
            spec['height_in'] = 30.0
            spec['width_in'] = 38.0
            spec['depth_in'] = 11.0
        else:
...
def parse_evaporator_coil(pdf_path: Path, model_number: str, orientation: str) -> Dict:
    ...
    # Typical dimensions for evaporator coils
    if spec['tonnage']:
        if orientation == 'Horizontal':
            # Horizontal coils are wider and shorter
            spec['height_in'] = 12.0 + (spec['tonnage'] * 2)
            spec['width_in'] = 18.0 + (spec['tonnage'] * 2)
            spec['depth_in'] = 20.0 + (spec['tonnage'] * 1.5)
            spec['cabinet_width_in'] = spec['width_in']
        else:  # Upflow / Vertical
            # Upflow coils are taller and narrower
            spec['height_in'] = 24.0 + (spec['tonnage'] * 3)
            spec['width_in'] = 14.5 + (spec['tonnage'] * 1)
            spec['depth_in'] = 17.5 + (spec['tonnage'] * 1)
            spec['cabinet_width_in'] = spec['width_in']
