#!/usr/bin/env python3
"""
Generate product descriptions for Goodman R-32 AC units
"""
...
def generate_ac_description(model_data):
    ...
    refrigerant = model_data['refrigerant_type']
...
<head>
    ...
    <title>Goodman {model_number} {tonnage_str} Ton R-32 Air Conditioner</title>
    <style>
        * {{
...
<body>
    <div id="product-container">
        <h1>Goodman {model_number} {tonnage_str} Ton R-32 Air Conditioner</h1>

        <div class="intro-section">
            <p>The Goodman {model_number} is a {tonnage_str}-ton R-32 air conditioner condenser from the {series} series, delivering {seer2} SEER2 cooling efficiency. Featuring a {compressor} compressor and environmentally-friendly R-32 refrigerant, this unit provides reliable cooling performance with improved efficiency for residential comfort applications.</p>
        </div>
        ...
        <div class="cta-section">
            ...
            <div class="compliance-badge">
                ...
                <div class="badge-content">
                    ...
                    <p>This complete R-32 system meets Department of Energy efficiency requirements for residential HVAC installations in Northern U.S. climate zones</p>
                </div>
            </div>
...
# Filter for AC condensers only (GLXS3BN series)
ac_models = {k: v for k, v in specs_data.items() if v['equipment_type'] == 'Air Conditioner Condenser'}

# Generate descriptions for each AC unit
generated_files = []
for model_number, model_data in ac_models.items():
    # Skip the 1.5 ton unit since it's already created
    if model_number == 'GLXS3BN1810':
        print(f"Skipping {model_number} (already exists)")
        continue

    html_content = generate_ac_description(model_data)

    filename = f"Goodman_R32_AC_{model_number}.html"
...
print(f"\n✓ Successfully generated {len(generated_files)} AC unit descriptions")
print(f"✓ Total AC units (including existing 1.5 ton): {len(ac_models)}")
